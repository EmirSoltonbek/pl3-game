...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `СЧЕТ: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `РАЗБИТО КУБИКОВ: ${state.game.cubeCount}`;
}
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				...
				highScoreLblNode.textContent = `Рекорд: ${formatNumber(getHighScore())}`;
			}
